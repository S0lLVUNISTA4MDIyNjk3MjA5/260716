...
グルーピングによって、これまで人間が選んだ1例（温度）だけでなく、**5つの概念グループすべてに`coverageGap()`を自動適用**した結果、温度以外のほぼ全てのケースで疑わしい、あるいは明確に誤った結果が出た。この問題は、下記5章の検討を経て工程4a（`quantity_extraction_prototype.js`）v2.4で修正済みである。以下は発見時の記録として残す。
...
5章の比較方向の設計判断は決着済み（v2.4で修正・回帰テスト済み）。残る検討候補は次のとおり。
...
4. 条件候補（`role=condition`）を、意味対応付けの観点でどう活用するか（現状はグルーピングに参加するだけで、比較には使われていない）
