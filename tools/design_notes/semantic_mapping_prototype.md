...
**作成日:** 2026-07-18
**版:** 2.6（1〜7章：概念候補・役割候補・比較方向の設計判断。8章：`interval_semantics`候補生成をv2.9で追加、外部レビューの必須修正2件をv2.10で反映、再レビューによる再指摘をv2.11で反映、承認後の推奨事項として「肯定的根拠」の設計をv2.12で追加、実務用語調査に基づく辞書拡張をv2.13で追加、キーワードのセル内スコープ修正をv2.14で反映、位置特定・要求側の共有述語の必須修正2件をv2.15で反映）
