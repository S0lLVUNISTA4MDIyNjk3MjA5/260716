...
**付属ファイル:** `tools/design_notes/semantic_mapping_prototype.js`（Node.js、依存ライブラリなし。工程4aを`require`する。`node semantic_mapping_prototype.js`で実行可能。完了条件アサーション39件を出力する）
...
`node semantic_mapping_prototype.js`で次を確認できる（8章の`interval_semantics`関連を含め、全体で39件中39件成功。この節は4〜5章に対応する10件）。
