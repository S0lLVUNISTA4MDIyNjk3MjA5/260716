...
- [x] 【v2.5で追加確認】片側区間（保証下限/保証上限）は自動判定せず`comparable:false`を返す安全策が、工程4a側で36件のアサーション全件成功として反映済み（5.1節、`quantity_extraction_prototype.md`5.8節参照）

## 7. 次の検討候補

5章の比較方向の設計判断は、v2.4での修正提案が外部レビューにより不十分と指摘され、v2.5で安全策（片側区間は自動判定しない）に見直された（5.1節）。恒久対応は次のとおり最優先で残っている。
