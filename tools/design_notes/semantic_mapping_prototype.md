...
**作成日:** 2026-07-18
**版:** 2.5（1〜7章：概念候補・役割候補・比較方向の設計判断。8章：`interval_semantics`候補生成をv2.9で追加、外部レビューの必須修正2件をv2.10で反映、再レビューによる再指摘をv2.11で反映、承認後の推奨事項として「肯定的根拠」の設計をv2.12で追加、実務用語調査に基づく辞書拡張をv2.13で追加、キーワードのセル内スコープ修正をv2.14で反映）
...
**付属ファイル:** `tools/design_notes/semantic_mapping_prototype.js`（Node.js、依存ライブラリなし。工程4aを`require`する。`node semantic_mapping_prototype.js`で実行可能。完了条件アサーション69件を出力する）
...
`node semantic_mapping_prototype.js`で次を確認できる（8章の`interval_semantics`関連を含め、全体で69件中69件成功。この節は4〜5章に対応する10件）。
