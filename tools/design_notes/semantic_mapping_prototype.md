...
**作成日:** 2026-07-18
**版:** 2.2（1〜7章：概念候補・役割候補・比較方向の設計判断。8章：`interval_semantics`候補生成をv2.9で追加、外部レビューの必須修正2件をv2.10で反映、再レビューによる再指摘をv2.11で反映）
...
**付属ファイル:** `tools/design_notes/semantic_mapping_prototype.js`（Node.js、依存ライブラリなし。工程4aを`require`する。`node semantic_mapping_prototype.js`で実行可能。完了条件アサーション41件を出力する）
...
`node semantic_mapping_prototype.js`で次を確認できる（8章の`interval_semantics`関連を含め、全体で41件中41件成功。この節は4〜5章に対応する10件）。
...
- **達成値であることの積極的な根拠が不足している**：v2.11時点では、周辺語・修飾語による裏付けのない点は、実仕様側であっても自動適用されない設計にした。これは安全だが、HVACサンプルの実データの大部分（冷房能力・電圧・騒音）が「要確認」に留まることを意味する。達成値であることを積極的に示す語彙（「実測」「達成」「検討結果である」等）の追加、または列名（`検討結果`/`標準機種情報`)自体を根拠として使う設計（`検討結果`列はより確信度の高い達成値候補として扱う等）を検討する必要がある
