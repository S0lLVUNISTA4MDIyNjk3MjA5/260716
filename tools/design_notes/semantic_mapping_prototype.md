...
### 8.5 実データでの検証結果（v2.11時点）

HVACサンプルの6概念グループ（PDF要求を持つ5グループ）に、`interval_semantics`候補生成→`comparisonMode`導出→`auto_applicable`判定→（適用可能な場合のみ）`coverageGap()`のパイプラインを適用した結果は次のとおり。**v2.11でachieved_pointの`source_role`重みを0.15→0.05へさらに引き下げたため（8.9節）、以前（v2.9〜v2.10）は自動適用されていた冷房能力・電圧・騒音の比較も、現在は「候補は生成されるが確信度不足のため要確認」に変わっている**。
...
| 冷房能力 | 要求 vs 標準機種／検討結果 | `achieved_point`（0.35、キーワードなし） | `point_in_region`（0.35） | false | 要確認（mode確信度0.35が閾値0.4未満） |
| 電源電圧 | 要求 vs 標準機種／検討結果 | `achieved_point`（0.35、キーワードなし） | `point_in_region`（0.35） | false | 要確認（同上） |
| 周波数 | 要求 vs 標準機種／検討結果 | — | — | false | 要確認（下記参照） |
| 運転騒音 | 要求 vs 標準機種／検討結果 | `achieved_point`（0.35、キーワードなし） | `point_in_region`（0.35） | false | 要確認（同上） |
...
冷房能力・電圧・騒音（要求 vs 標準機種／検討結果いずれも）は、v2.11時点では**「点である」構造的根拠と「実仕様側である」役割根拠だけでは自動適用の閾値（modeConfidence0.4）に届かない**設計になったため、すべて「要確認」になる。これはv2.10までの結果からの後退に見えるが、外部レビューにより「否定語辞書にない未知の曖昧語（代表値・中央値・計画値等）を伴う点も、この2つの根拠だけでは区別できず自動適用されてしまう」という指摘を受けての、意図的かつ安全側の変更である（8.9節）。**現時点のHVACサンプルには、Excelセル内に「達成値である」ことを明示する語（実測・達成・検討結果である旨等）が無いため、この3概念は候補生成はできても自動適用はできない状態にある。** 恒久対応（達成値であることを積極的に示す語彙の追加、または列名自体を根拠として使う設計等）は8.10節の残課題とする。
