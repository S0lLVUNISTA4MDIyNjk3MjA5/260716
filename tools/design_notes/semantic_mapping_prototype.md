...
**作成日:** 2026-07-18
**版:** 2.3（1〜7章：概念候補・役割候補・比較方向の設計判断。8章：`interval_semantics`候補生成をv2.9で追加、外部レビューの必須修正2件をv2.10で反映、再レビューによる再指摘をv2.11で反映、承認後の推奨事項として「肯定的根拠」の設計をv2.12で追加）
...
**付属ファイル:** `tools/design_notes/semantic_mapping_prototype.js`（Node.js、依存ライブラリなし。工程4aを`require`する。`node semantic_mapping_prototype.js`で実行可能。完了条件アサーション45件を出力する）
...
`node semantic_mapping_prototype.js`で次を確認できる（8章の`interval_semantics`関連を含め、全体で45件中45件成功。この節は4〜5章に対応する10件）。
...
`comparisonMode`の導出テーブルは、レビューが例示した組み合わせをベースに、v2.10で1行削除して採用した（削除の経緯は8.8節参照。**`required_capability_domain`×`achieved_point`は導出規則なし**であることに注意。要求が対応可能領域全体を求めているのに対し、実仕様が単一の達成値では要求範囲全域への対応を証明できないため）。
...
テーブルにない組み合わせ（`required_capability_domain`×`achieved_point`を含む）、またはどちらかの最上位候補が`unknown`の場合は`comparisonMode`を導出しない（`null`を返す。推測しない）。
...
### 8.5 実データでの検証結果（v2.12時点）

HVACサンプルの6概念グループ（PDF要求を持つ5グループ）に、`interval_semantics`候補生成→`comparisonMode`導出→`auto_applicable`判定→（適用可能な場合のみ）`coverageGap()`のパイプラインを適用した結果は次のとおり。**v2.11でachieved_pointの`source_role`重みを0.15→0.05へ引き下げ、v2.12で`source_role`（`ctx.side==='B'`）を`column_role`（`ctx.sourceColumn==='検討結果'`）へ置き換えたため（8.9〜8.11節）、標準機種情報列と検討結果列とで確信度がわずかに異なる（0.30 / 0.35）。いずれも自動適用の閾値（0.4）には届かないため、以前（v2.9〜v2.10）は自動適用されていた冷房能力・電圧・騒音の比較は、標準機種／検討結果のどちらも「候補は生成されるが確信度不足のため要確認」のままである**。
...
| 冷房能力 | 要求 vs 標準機種 | `achieved_point`（0.30、キーワードなし・列根拠なし） | `point_in_region`（0.30） | false | 要確認（mode確信度0.30が閾値0.4未満） |
| 冷房能力 | 要求 vs 検討結果 | `achieved_point`（0.35、キーワードなし・列根拠あり） | `point_in_region`（0.35） | false | 要確認（mode確信度0.35が閾値0.4未満） |
| 電源電圧 | 要求 vs 標準機種 | `achieved_point`（0.30、キーワードなし・列根拠なし） | `point_in_region`（0.30） | false | 要確認（同上） |
| 電源電圧 | 要求 vs 検討結果 | `achieved_point`（0.35、キーワードなし・列根拠あり） | `point_in_region`（0.35） | false | 要確認（同上） |
| 周波数 | 要求 vs 標準機種 | `unknown`（0.15、非interval形式） | 導出不可 | false | 要確認（下記参照） |
| 周波数 | 要求 vs 検討結果 | `achieved_point`（0.35、キーワードなし・列根拠あり） | `point_in_region`（0.35） | false | 要確認（設計特性の対応確信度不足。下記参照） |
| 運転騒音 | 要求 vs 標準機種 | `achieved_point`（0.30、キーワードなし・列根拠なし） | `point_in_region`（0.30） | false | 要確認（同上） |
| 運転騒音 | 要求 vs 検討結果 | `achieved_point`（0.35、キーワードなし・列根拠あり） | `point_in_region`（0.35） | false | 要確認（同上） |

温度の「要求 vs 標準機種」（Excel原文「0 °C～40 °C」）は、`使用可能`等の明示語を含まないため`interval_semantics`が`unknown`となり、`comparisonMode`を導出できず「要確認」のまま留まった。これは**バグではなく意図した挙動**であり、v2.5〜v2.6で確立した「区間の意味が確定できない場合は自動判定しない」という原則を、`interval_semantics`候補生成の入力データそのものが薄い場合にも一貫して適用した結果である。

冷房能力・電圧・騒音（要求 vs 標準機種／検討結果いずれも）は、v2.12時点では**「点である」構造的根拠（0.30）に「検討結果列である」という列根拠（+0.05、標準機種情報列には付かない）を加えても、自動適用の閾値（modeConfidence0.4）に届かない**設計になったため、標準機種・検討結果のどちらも「要確認」になる。これはv2.10までの結果からの後退に見えるが、外部レビューにより「否定語辞書にない未知の曖昧語（代表値・中央値・計画値等）を伴う点も、構造的根拠だけでは区別できず自動適用されてしまう」という指摘を受けての、意図的かつ安全側の変更である（8.9節）。v2.12では、セル内に達成値であることを積極的に示す語（実測・達成値・検討の結果等）がある場合のみ自動適用できる仕組みを追加したが（8.11節）、**現時点のHVACサンプルにはそのような語が無いため、この3概念は候補生成はできても自動適用はできない状態のままである。**
...
### 8.9 再レビューによる再指摘：必須修正2は本質的に未完（v2.11）

v2.10を再レビューへ提出したところ、「必須修正1は完了、必須修正2はまだ本質的に未完」との判定を受けた。あわせて、8.3節・8.5節の記述がv2.10のコード変更を反映しておらず、文書とコードが矛盾しているという指摘も受けた。指摘は着手前に実際のコードで再現を確認した（推測で対応せず、レビューの主張を裏取りした上で着手）。**両指摘とも実際に再現した。**

**再指摘の要旨**：v2.10で`achieved_point`の`source_role`重みを0.3→0.15へ下げ、「設定」「公称」「試験」を否定根拠の語彙に追加したが、これは列挙した否定語に一致する場合だけ効果を発揮する対症療法だった。`achieved_point`の基本スコアは依然として`点である(0.3)+B側である(0.15)=0.45`であり、`modeConfidence`閾値（0.4）を超え、`unknown`（0.15）との差も0.3ある。否定語辞書に**ない**曖昧な修飾語（「代表値」「中央値」「計画値」「参考設計値」等）を伴う点は、区別なく引き続き自動適用される。具体的には、要求「騒音60dB以下」×実仕様「代表値58dB」が、`point_in_region`で自動導出・自動適用され`satisfied:true`を返すことを確認した。これは「点形状＋B側という役割だけで達成値と判断する」という、当初の問題の構造そのものが温存されていたことを意味する。

**採用した対応**：レビューが提示した3案のうち、最も安全側に倒す案（`source_role`の重みを0.05以下にする）を採用した。

```js
{ value: 'achieved_point', weight: 0.05, evidenceType: 'source_role', ... }
```

これにより、無修飾の点（周辺語・修飾語による裏付けが一切ない点）は`achieved_point`確信度が`0.3+0.05=0.35`となり、候補としては最上位に残るものの、`modeConfidence`閾値（0.4）に届かず、`comparisonMode`候補自体は導出されても`auto_applicable:false`（要確認）になる。

**この修正の副作用**：HVACサンプルの実データのうち、冷房能力・電源電圧・運転騒音の3概念（要求 vs 標準機種／検討結果いずれも）は、Excelセル内に「達成値である」ことを明示する語を含まないため、v2.10までは自動適用されていたのが、v2.11では**すべて「要確認」に変わった**（8.5節の表を参照）。自動適用が維持されるのは、明示的な肯定キーワード（「使用可能」等）を持つ温度の「要求 vs 検討結果」のみになった。これは後退ではなく、レビューが明示的に許容したトレードオフである（「これなら無修飾の点は0.35となり、候補として表示されつつ自動適用はされません」）。人間の確認対象が増えることにはなるが、達成値であることを積極的に示す語彙（「実測」「達成」「検討結果である」等）を追加する、または列名自体（`検討結果`列に対する信頼度を`標準機種情報`列より高くする等）を根拠として使う設計は、次の検討候補として残した（8.10節）。

再レビューが提示した3件のテストケース（要求「騒音60dB以下」×実仕様「代表値58dB」、要求「0〜50℃で運転できること」×実仕様「中央値25℃」、要求「220Vとすること」×実仕様「計画値220V」）を、いずれも`generateIntervalSemanticsCandidates()`の候補順位だけでなく`evaluateAutoApplicable()`まで通して確認し（`comparisonMode`候補自体は導出されるが`auto_applicable:false`になることを確認）、既存36件に追加した。あわせて、v2.10までの4件の必須修正2回帰テストのうち1件（冷房能力/検討結果が「自動適用される」ことを検証していたテスト）を、v2.11の新しい挙動（「候補は導出されるが自動適用されない」）に合わせて更新した。**合計39件の自動アサーションが全件成功**することを確認した。承認後、レビューから軽微な改善提案（`applicable:false`だけでなく`modeCandidate`の中身（`point_in_region`・確信度0.35）も固定しておくと回帰原因を判別しやすい）を受け、要求側が`acceptable_region`となる2例（代表値58dB、計画値220V）にその確認を追加し、**合計41件**に更新した。

また、8.3節の`comparisonMode`導出テーブルと8.5節の実データ結果表が、v2.10のコード変更（`required_capability_domain`×`achieved_point`の削除）およびv2.11の重み変更を反映していなかった点も、本節の対応にあわせて修正した。

### 8.10 残課題（v2.11時点。「達成値の積極的根拠」はv2.12で着手。8.11節を参照）

- ~~**達成値であることの積極的な根拠が不足している**~~：v2.12で着手した（8.11節）
- **`comparisonMode`候補の直積化（推奨修正3、未対応）**：現状は要求側・実仕様側それぞれの最上位候補のペアしか見ていない。最上位同士は未定義でも、次点候補同士なら有効な対応があるという情報を活かすには、直積から`comparison_mode_candidates`を複数生成し確信度順に返す設計が必要
- **要求側の`±`表記**：8.7節から持ち越し。要求側自体が公差表記であるケースの扱いは未検証（HVACサンプルに実例がないため）
- **概念辞書・確信度スコアリングとの相互作用**：8.7節から持ち越し
- **条件節（`role=condition`）の活用**：8.7節から持ち越し
- **`lowGap`/`highGap`の表現見直し**：8.7節から持ち越し

### 8.11 肯定的根拠（列の役割＋セル内の肯定語＋数量形状）の設計（v2.12）

v2.11の承認にあわせ、レビューから「列の役割＋セル内の肯定語＋数量形状を独立根拠として積み上げる設計」が次の推奨として提示された。これに沿って`achieved_point`のスコアリングを再設計した。

**当初案とその危険性**：最初に検討したのは、v2.11で`0.05`まで下げた`source_role`（`ctx.side==='B'`）を、より具体的な`column_role`（`ctx.sourceColumn==='検討結果'`）に置き換え、重みをある程度大きくする（例：`0.15`）案だった。しかし実装前に検証したところ、この案には重大な欠陥があることが分かった。「点である」（0.3）＋「検討結果列である」（0.15）＝0.45となり、**否定語辞書にない未知の曖昧語（代表値・中央値・計画値等）が「検討結果」列に置かれた場合、v2.11で塞いだはずの脆弱性がそのまま再発する**ことを、実装前のスクラッチ検証で確認した。列名という構造的な手掛かりを強めるほど、v2.9の根本問題（「形＋役割だけで達成値と判断する」）を再現するリスクが高まるという、当初案の矛盾に気づいた。

**採用した設計**：構造的根拠（数量の形・列名）は弱いまま維持し、**セル内の肯定語（「実測」「達成値」「検討の結果」「測定した結果」等）だけが、自動適用の閾値を超えられる強い根拠（重み0.4）を持つ**という非対称な設計にした。

- `quantity_shape`（点である）：0.3（変更なし）
- `column_role`（出典列が`検討結果`）：0.05（`source_role`から改名。重みは維持）
- `keyword`（セル内に達成値を示す語）：0.4（新規追加）

無修飾の点は「形＋列根拠」で最大`0.35`にしかならず、自動適用の閾値（`0.4`）に届かない。列を`標準機種情報`にしても`検討結果`にしても、この上限は変わらない（`標準機種情報`は列根拠が付かず`0.3`のまま）。一方、明示的な肯定語がある場合は「形＋肯定語」で`0.7`に達し、`auto_applicable`の条件を満たせば自動適用される。

**この設計変更による実データへの影響**：HVACサンプルの冷房能力・電源電圧・運転騒音（Excelセルに「実測」等の語を含まない）は、v2.12でも引き続き「要確認」のままであり、8.5節の実データ結果表に変化はない。今回追加したのは、将来より詳細な語彙を持つExcelデータ（例：「実測値：12.5 kW」のような表記）が与えられた場合に、安全に自動適用へ昇格できる**インフラストラクチャ**であり、現在のHVACサンプルの挙動自体を変えるものではない。

既存41件に4件（セル内肯定語による自動適用の確認2件、列根拠だけでは閾値に届かないことの確認2件）を追加し、既存の必須修正2(再指摘)回帰テスト（代表値・中央値・計画値の3例）は、`sourceColumn:'検討結果'`を明示して再検証するよう更新した（列根拠を組み合わせても安全であることの、より厳密な確認になる）。**合計45件の自動アサーションが全件成功**することを確認した。
