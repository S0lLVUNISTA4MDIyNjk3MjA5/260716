...
**作成日:** 2026-07-18
**版:** 2.4（1〜7章：概念候補・役割候補・比較方向の設計判断。8章：`interval_semantics`候補生成をv2.9で追加、外部レビューの必須修正2件をv2.10で反映、再レビューによる再指摘をv2.11で反映、承認後の推奨事項として「肯定的根拠」の設計をv2.12で追加、実務用語調査に基づく辞書拡張をv2.13で追加）
...
**付属ファイル:** `tools/design_notes/semantic_mapping_prototype.js`（Node.js、依存ライブラリなし。工程4aを`require`する。`node semantic_mapping_prototype.js`で実行可能。完了条件アサーション57件を出力する）
...
v2.12承認後、レビューから「肯定語の種類を増やす前に、実際の客先・設計文書から匿名化した表現例を集め、誤昇格率を確認する段階へ進むのが妥当」との提案があった。実際のサンプル文書は用意できなかったため、代わりにインターネット検索でJIS計測用語・検査成績書の実務慣行を調査し、それを根拠として語彙を拡張する方針を採った。

**調査結果の要約**（詳細な出典は本節末尾）：

| 語 | 意味 | 分類 |
|---|---|---|
| 実測値・測定値 | 実際に測定して得た値。理論値・推測値との対比で使われる | 達成値（肯定語、対応済み） |
| 試験結果・検査結果 | 実施した試験・検査で得た値。実測と同義 | 達成値（肯定語、v2.13で追加） |
| 実績値・成績値 | 検査成績書等に記載される、実際の測定・判定結果 | 達成値（肯定語、v2.13で追加） |
| 公称値 | 設計時に定めた性能表示値。理想条件下の値であり実測とずれ得る | 非達成値（否定語、v2.10で対応済み） |
| 規格値・基準値 | 品質特性の許容範囲・判定基準として設定される値 | 非達成値（否定語、v2.13で追加） |
| 目標値 | 開発段階の目安。社内管理基準として規格値よりさらに厳しく設定されることもある | 非達成値（否定語、v2.13で追加） |
| 設計値 | 設計段階での目標値・理想値。安全性・機能・信頼性・コストのバランスから決まる | 非達成値（否定語、v2.13で追加） |
| 代表値・中央値・平均値 | 統計用語（JIS Z 8101-1）。複数データを要約する値（平均・中央値・最頻値等の総称） | **意図的に未分類のまま**（後述） |

**採用した対応**：

- 肯定語（`achieved_point`、重み0.4）へ「試験結果」「検査結果」「実績値」「成績値」を追加した。ただし「試験結果」は既存の否定語「試験」の正規表現に単純に一致してしまうため、否定語側の正規表現を`試験(?!結果)`（「試験」の直後が「結果」でない場合のみ）へ変更し、「試験結果」が誤って否定根拠に巻き込まれないようにした。
