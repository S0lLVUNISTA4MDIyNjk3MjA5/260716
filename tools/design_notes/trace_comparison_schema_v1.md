...
- `hash_algorithm`：`content_hash`/`quantity_id`/`dataset_signature`すべてに共通で使ったアルゴリズム名。将来アルゴリズムを変更する場合の互換性判定に使う。
...
```
quantity_id = sha256([trace_id, source_field, source_span.start, source_span.end, normalized_text].join(""))の先頭N桁
...
**改訂経緯**：以前の版は`baseline_v1_handoff.md` §9の手打ちサンプル（架空の`reqRec`/`actRec`）と、実ブラウザ検証で使ったtrace_id（`design-use-temperature`）を組み合わせて作られており、`actual_ref.trace_id`（使用温度範囲のレコード）と`actual_analysis.quantity`（冷房能力12.5 kWの値）が実際には無関係という不整合があった（レビュー指摘）。「JSONとしてparseできる」ことは構文面の確認に過ぎず、`trace_id`・数量・概念が同じ元レコードを指すという**参照整合性**の検証にはならない、との指摘も受けた。

このため、手打ちの値を一切使わず、`tools/design_notes/trace_comparison_example_verification.js`が実データ（`samples/hvac_trace_sample_small/`）を読み込み、プロトタイプの実関数（`extractQuantities()`・`buildPropertyCandidateRecords()`・`deriveComparisonModeCandidate()`・`evaluateAutoApplicable()`・`coverageGap()`）へそのまま通して機械生成したレコードに差し替えた。22件のアサーションで、`requirement_ref`/`actual_ref`の`trace_id`が実データの`trace_id`と一致すること、`quantity.source_text`が元の`source_raw_text`/`source_record`の部分文字列であること、`mapping.concept_id`が実際に「冷房能力」に対応する`concept_id`であることを機械検証している（`node tools/design_notes/trace_comparison_example_verification.js`で再実行できる。出力は`tools/design_notes/runtime_fixtures/trace_comparison_example_verified.json`）。

要求側`req-cooling-capacity`（PDF、trace_text:「周囲温度50 °Cにおいて、冷房能力12 kW以上を確保すること。」）と、実仕様側`design-cooling-capacity`（Excel、`検討結果`列:「周囲温度50 °Cで12.5 kW」）を実際に照合した結果：

```json
{
  "schema_version": "trace-comparison/1.0-rc1",
  "generated_at": "2026-07-19T08:54:58.963Z",
  "note": "trace_comparison_example_verification.jsにより実データから機械的に生成・検証済み。手打ちの値は含まない。",
  "source": {
  ...
  "comparisons": [
    {
      "comparison_id": "req-cooling-capacity::design-cooling-capacity::q-be1c0825cbf56b0f::q-017849d7273ef6a1",
      ...
      "quantity_pair_id": "q-be1c0825cbf56b0f::q-017849d7273ef6a1",
      "relationship": {
        "source": "matching_engine", "match_method": "tag", "match_confidence": 0.88,
        "review_category": "要確認", "linked_at": null
      },
      "requirement_analysis": {
        "quantity_id": "q-be1c0825cbf56b0f", "source_field": "source_raw_text", "occurrence_index": 0,
        "source_span": { "start": 18, "end": 23 }, "content_hash": "92e148aaaf322f2d",
        "quantity": {
      ...
      "actual_analysis": {
        "quantity_id": "q-017849d7273ef6a1", "source_field": "検討結果", "occurrence_index": 0,
        "source_span": { "start": 10, "end": 17 }, "content_hash": "f9e2561cc644c598",
        "quantity": {
