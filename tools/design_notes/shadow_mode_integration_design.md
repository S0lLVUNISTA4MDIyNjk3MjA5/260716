...
```
quantity_id = "q-" + sha256([trace_id, source_field, source_span.start, source_span.end, normalized_text].join(""))の先頭N桁
```
  ...
  "records": [
    {
      ...
      "content_hash": "92e148aaaf322f2d...(64hex)",
      "analyses": [
        {
...
- `hash_algorithm`：`dataset_signature`/`content_hash`/`quantity_id`に共通で使ったアルゴリズム名（`"SHA-256"`固定、2.0節参照）。
...
5. `trace_comparison_schema_v1.md` §11の完全な具体例が、実データからの機械生成・機械検証で作られていること（`trace_comparison_example_verification.js`で達成済み。今後スキーマ側に変更があった場合は再実行して検証を保つ）
6. 8節の未解決事項のうち、少なくとも概念辞書とコード共有方式について実装方針が決まっていること（完全解決までは求めないが、「未定」のままでの正式版化はしない）

**この改訂までに完了した項目**（上記条件との対応）：条件5（§11の機械検証）は完了。条件1〜4・6は未達（回帰テストは設計のみで未実装、シグネチャ変更は未着手、本体コード実装は未着手）。
